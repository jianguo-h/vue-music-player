	...
	export default {
		...
		watch: {
			...
			curLrcIndex(newCurLrcIndex, oldCurLrcIndex) {
				...
				let lrcBoxHeight = lrcBox.offsetHeight;
				let childHeight = lrcBox.firstChild.offsetHeight;
				let curShowNum = Math.floor(lrcBoxHeight / childHeight);
		...
		methods: {
			// 初始化播放信息
			initPlay() {
				let songData = this.songList[this.curPlayIndex];
			...
			// 歌词滚动
			lrcRoll() {
				let curPlayLrcArr = this.curPlayLrcArr;
				let currentTime = Number(this.audio.currentTime.toFixed(2));
			...
			dealMode() {
				...
				if(this.modeType === "random") {
					let randomIndex = Math.floor(Math.random() * this.listTotal);
			...
			changeLrcColor(index) {
				let currentItem = this.lrcColorList[index];
