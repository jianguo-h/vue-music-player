<template>
  <div class="banner" @touchstart="touchstart" @touchmove="touchmove" @touchend="touchend">
    ...
    >
      <div class="banner-item" :style="{marginRight: itemSpacing + 'px'}" v-for="(item, index) of items" :key='index'>
        <img :src="item" width="100%">
      </div>
    </div>
    <div class="banner-pagination" v-if="initialItems.length > 0">
      <span :class="{active: curPaginationIndex === index}" v-for="(inititalItem, index) in initialItems" :key='index' @click="translate(index)">
      </span>
    </div>
  </div>
</template>
